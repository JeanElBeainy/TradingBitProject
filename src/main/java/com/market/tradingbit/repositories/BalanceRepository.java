package com.market.tradingbit.repositories;

import com.market.tradingbit.entities.Balance;
import jakarta.transaction.Transactional;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Modifying;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;

import java.math.BigDecimal;


public interface BalanceRepository extends JpaRepository<Balance, Long> {

    @Modifying
    @Transactional
    @Query("UPDATE Balance b SET b.usdBalance = :amount," +
            "b.totalBalance = (b.usdBalance + b.cryptoBalance +b.stockBalance)" +
            "WHERE b.id = :userId")
    void updateUSDBalanceByAmountAndUserId(@Param("amount") BigDecimal amount, Long userId);

    @Modifying
    @Transactional
    @Query("UPDATE Balance b SET b.cryptoBalance = :amount," +
            "b.totalBalance = (b.usdBalance + b.cryptoBalance +b.stockBalance)" +
            " WHERE b.id = :userId")
    void updateCryptoBalanceByAmountAndUserId(@Param("amount") BigDecimal amount, Long userId);

    @Modifying
    @Transactional
    @Query("UPDATE Balance b SET b.totalVolume = (b.totalVolume + :volume) WHERE b.id = :userId")
    void updateTotalVolumeByAmountAndUserId(@Param("volume") BigDecimal volume, Long userId);
}
